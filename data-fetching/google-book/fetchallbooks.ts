...
let iter =1
let ERRORS =0
async function fetchFirstBookId(title) {
    const url = generateTitleSearchUrl(`${title} by ${books[title].author}`)
    ...
    if (firstItem.id) {
        ...
        console.log(`${iter}/${len} ${title} --> ${firstItem.volumeInfo.title}` )
        books[title].id = id
        if (firstItem.volumeInfo.title !== title) {
            errors[title] = firstItem.volumeInfo.title
        }
    }   
    else {
        console.log(`${iter}/${len} ${title} --> ${firstItem.volumeInfo.title} - ERROR` )
        books[title].id = 'ERROR'
    }
    iter++
}
const generateTitleSearchUrl = (bookTitle, api='https://www.googleapis.com/books/v1/volumes?') => 
`${api}q=${bookTitle}`
let timer = 0
titles.forEach(title=> {
timer += 750
setTimeout(
    ()=>fetchFirstBookId(title),timer 
    )
})
